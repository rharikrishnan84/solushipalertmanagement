...
/**
 * MMRTrackingInterceptor class is an interceptor class to to track and persists httpServletRequest
 * related parameters and details in the database.
 * <p/>
...
public class MmrTrackingInterceptor extends ExceptionMappingInterceptor {
  ...
  public final String intercept(ActionInvocation invocation) throws Exception {
    ...
    int trackingOn = Integer.parseInt(WebUtil.getProperty(Constants.SYSTEM_SCOPE,
        Constants.TRACKING_ON));
    ...
    UserTrackingSessionListener userSessionListener = (UserTrackingSessionListener) ActionContext
    ...
    if (userSessionListener == null) {
      ...
      ActionContext.getContext().getSession()
          .put(Constants.USER_TRACKING_SESSION_LISTENER, userSessionListener);
    }
  ...
  /**
   * trackRequestURL method, tracks and persists httpServletRequest details in the database
   * 
   * @param p_invocation
  ...
  private RequestDetailsVO trackRequestURL(ActionInvocation p_invocation) {
    ...
    HttpServletRequest httpServletRequest = (HttpServletRequest) p_invocation
    ...
    try {
      ...
      l_requestDetailVo.setActionName(p_invocation.getInvocationContext().getName());
      l_requestDetailVo.setRemoteAddr(httpServletRequest.getRemoteAddr());
      l_requestDetailVo.setRemoteHost(InetAddress.getByName(httpServletRequest.getRemoteAddr())
          .getHostName());
      ...
      if (user != null && user.getTimeZone() != null && !user.getTimeZone().isEmpty()) {
        ...
        Date requestDate = dateFormat.parse(dateFormat.format(cal.getTime()));
        l_requestDetailVo.setRequestDate(requestDate);
      } else {
      ...
      log.debug((new StringBuilder("Exception while saving httpServletRequest Info: ")).append(ex)
          .toString(), ex);
    }
    return l_requestDetailVo;
  }
  ...
  /**
   * trackRequestParams method, tracks and persists request parameter according to request track
   * metadata info
   * 
   * @param invocation
  ...
  private List<RequestTrackParamVO> trackRequestParams(ActionInvocation p_invocation) {
    ...
    WebApplicationContext webApplicationContext = WebApplicationContextUtils
        .getWebApplicationContext((ServletContext) p_invocation.getInvocationContext().get(
            ServletActionContext.SERVLET_CONTEXT));

    // instantiating requestTrackingDao
    RequestTrackingDAO l_requestTrackingDao = (RequestTrackingDAO) webApplicationContext
        .getBean(REQUEST_TRACKING_DAO);
    // list of parameters that are supposed to be tracked
    List<RequestTrackMetadataVO> l_trackingMetaDataList = l_requestTrackingDao
        .getRequestParamMetadata();
  ...
  /**
   * processApplicationParams method, tracks and persists application specific parameters
   * 
   * @param invocation
   * @return requestTrackParamVOList
   */
