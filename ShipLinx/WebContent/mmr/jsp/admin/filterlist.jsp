	...
	<script type="text/javascript">
		function editSearchFilter(){
		   ...
		   if(txt1 < 1){
			...
			document.getElementById("searchUserFormId").action = "edit.filter.action?method1=edit&getcus=getting&name1="+txt;
		   ...
		   if(txt1 < 1){
			...
			alert('Please check atmost one');
				...
				$('#confirmBtn').click(function(){
					removeCustomConfirm();
					document.getElementById("searchUserFormId").action = "delete.filter.action?name="+txt;
					document.getElementById("searchUserFormId").submit();
				});
...
<s:form id="searchUserFormId" cssClass="form" >
	<div id="srchusr_results">	
		<div id="srchusr_res"><span>FILTER LIST</span></div>
		...
		<a href="#" id="actiondown" ><mmr:message messageId="label.action"/><span style="font-size:14px;">&#9660;</span></a>
			...
			<ul id="actionmenu">
			<li><a href="javascript: editSearchFilter();" ><mmr:message messageId="label.list.edit"/></a></li>
			<!--<s:submit value="DELETE" onclick="deleteSearchUser();" /> -->
			<li><a href="#" onclick="deleteSearchFilter();"><mmr:message messageId="label.list.delete"/></a></li>
			<li><a href="addFilter.action">ADD FILTER</a></li>
			</ul>
		</div>	
	</div>
...
<table cellpadding="0" cellspacing="0"  border="0px" class="display" id="sample1" style="float:left; width:100%; height:auto;">
    <thead>
		<tr>
			...
			<th></th>
			<th><span style="width:250px !important; float:left;">FILTER NAME</span></th>
			<th><span style="width:250px !important; float:left;">DESCRIPITION</span></th>
	...
	<tbody>
            <s:iterator id="usertable" value="filterList" status="rowstatus">
         
             <tr>
                 
       
		        <td class="odd1" width="2%">
		        <input  class="dataTable-checkbox" type="checkbox" name="searchUserCheckBox" value="<s:property value="filterId"/>"/>  
	   			</td>
				 <td width="2%">
				...
				<td style="text-align: left;"><span title="<s:property value="filterName"/>"></span><div style="width:170px;overflow:hidden;white-space:nowrap;text-overflow: ellipsis"><s:property value="filterName"/></div></td>
	            <td style="text-align: left;"> <span title="<s:property value="description"/>"></span><div style="width:200px !important;overflow:hidden;white-space:nowrap;text-overflow: ellipsis"><s:property value="description"/></div></td>
