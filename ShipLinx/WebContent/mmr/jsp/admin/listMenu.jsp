...
<SCRIPT language="JavaScript">
	...
	<script type="text/javascript">
		function addMenu(){
			document.getElementById("searchUserFormId").action = "add.menu.action";
			document.getElementById("searchUserFormId").submit();
...
<script>
	...
	<script type="text/javascript">
		function editMenu(){
			var editMenuId = document.getElementsByName("searchMenu");
			var i1,txt1 = 0;
		   for (i1=0;i1<editMenuId.length;i1++){
			if (editMenuId[i1].checked){
			 txt1 += 1;      
			}
		   }
		   if(txt1 < 1){
			...
			alert('Please check atmost one');
				...
				var i,txt;
				for (i=0;i<editMenuId.length;i++){
					if (editMenuId[i].checked){
						txt = editMenuId[i].value ;					
					}
				}
				document.getElementById("searchUserFormId").action = "edit.menu.action?editMenuId="+txt;
				document.getElementById("searchUserFormId").submit();
			}
		}
	</script>
		...
		function deleteMenu(){
		   ...
		   if(txt1 < 1){
			...
			alert('Please check atmost one');
				...
				document.getElementById("searchUserFormId").action = "delete.menu.action?editMenuId="+txt;
				document.getElementById("searchUserFormId").submit();
			}
		}
	</script>
...
<div class="content_table" style="background-color:#fff">
	...
	<div id="srchusr_res"><span><mmr:message messageId="label.menu.list"/></span></div>
		<div class="form_buttons" style="float:right;">
			...
			<ul id="actionmenu">
			<li><a href="#" onclick="addMenu();"><mmr:message messageId="label.menu.add"/></a></li>
			<li><a href="#" onclick="editMenu();"><mmr:message messageId="label.list.edit"/></a></li>
			<li><a href="#" onclick="deleteMenu();"><mmr:message messageId="label.delete"/></a></li>
			</ul>
		</div>	
	</div>
       <div id="result_tbl">
       ...
       <table cellpadding="0" cellspacing="0"  border="0px" class="display" id="sample1" width="100%">
		<thead>
			...
			<th style="width: 175px !important;">  <mmr:message messageId="label.level"/></th>
			<th style="width: 220px ;"> <mmr:message messageId="label.menu.parentid"/></th>
			<th style="text-align:right;padding-right:10px;"> <mmr:message messageId="label.business.url"/></th>
			</tr>
		</thead>
		<tbody>
			
          <s:iterator id="artable" value="menuVo" status="rowstatus">
             <tr>
