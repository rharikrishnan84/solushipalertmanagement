	...
	<script type="text/javascript">
		function editSearchPartner(){
		   ...
		   if(txt1 < 1){
			...
			document.getElementById("searchUserFormId").action = "edit.partner.action?method=edit&name="+txt;
			document.getElementById("searchUserFormId").submit();
			}
		}
	</script>
		<script type="text/javascript">
		function deleteSearchPartner(){
		   ...
		   if(txt1 < 1){
			...
			alert('Please check atmost one');
				...
				$('#confirmBtn').click(function(){
					removeCustomConfirm();
					document.getElementById("searchUserFormId").action = "delete.partner.action?name="+txt;
					document.getElementById("searchUserFormId").submit();
				});
...
<s:form id="searchUserFormId" cssClass="form" >
	<div id="srchusr_results">	
		<div id="srchusr_res"><span>Country List</span></div>
		...
		<a href="#" id="actiondown" ><mmr:message messageId="label.action"/><span style="font-size:14px;">&#9660;</span></a>
			...
			<ul id="actionmenu">
			<li><a href="javascript: editSearchPartner();" ><mmr:message messageId="label.list.edit"/></a></li>
			<!--<s:submit value="DELETE" onclick="deleteSearchUser();" /> -->
			<li><a href="#" onclick="deleteSearchPartner();"><mmr:message messageId="label.list.delete"/></a></li>
			<li><a href="addPartner.action"><mmr:message messageId="menu.admin.addPartner"/></a></li>
			</ul>
		</div>	
	</div>
...
<table cellpadding="0" cellspacing="0"  border="0px" class="display" id="sample1" style="float:left; width:100%; height:auto;">
    <thead>
		<tr>
			...
			<th></th>
			<th><span style="width:250px !important; float:left;"><mmr:message messageId="label.ghead.partnername"/></span></th>
	...
	<tbody>
            <s:iterator id="usertable" value="partnerList" status="rowstatus">
         
             <tr>
                 
       
		        <td class="odd1" width="2%">
		        <input  class="dataTable-checkbox" type="checkbox" name="searchUserCheckBox" value="<s:property value="partnerId"/>"/>  
	   			</td>
				 <td width="2%">
				...
				<td style="text-align: left;"><span title="<s:property value="partnerName"/>"></span><div style="width:170px;overflow:hidden;white-space:nowrap;text-overflow: ellipsis"><s:property value="partnerName"/></div></td>
